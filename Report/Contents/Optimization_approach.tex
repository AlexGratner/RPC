...
GP is considered to be promising in solving optimization problems of mechatronic system design since many of the constraints can be expressed by polynomial approximation. Although there is some distance between posynomials and polynomials, it is possible to get posynomial form while since the parameters we deal with are all in positive orthant and all exponents in posynomials can be any real numbers while only nonnegative integers for polynomial models. Many properties of the mechanical components have been summarized as lookup tables. In order to create constraints, many of those properties need to be expressed by objective variables posynomials. In fact, some of the models in \cite{roos2007towards} are achieved by data fitting and they end up as posynomials. 
